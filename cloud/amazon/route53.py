...
def main():
    ...
    if command_in == 'create_hosted_zone':
        route53 = Route53Connection(aws_access_key, aws_secret_key)
        response = route53.create_hosted_zone(zone_in)
        if response is None:
            module.fail_json(msg = "unable to create the specified hosted zone")
        else:
